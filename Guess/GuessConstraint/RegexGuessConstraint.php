 ...
 */
class RegexGuessConstraint extends AbstractGuessConstraint
{
    ...
    /**
     * @param Constraint|Regex $constraint
     */
    public function guess(ChildMetadataBuilderInterface $builder, Constraint $constraint): void
    {
        $this->addType($builder, '?string');
        $this->addInput($builder, '?text', !$constraint->match ? [] : [
            'regex_match' => $constraint->match,
            'regex_pattern' => $constraint->pattern,
            'regex_html_pattern' => $constraint->htmlPattern,
        ]);
    }
}
